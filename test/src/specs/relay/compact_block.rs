...
impl CompactBlockBasic {
    ...
    pub fn test_lose_get_block_transactions(
        ...
        node0.connect(node1);
        assert!(wait_until(20, || node0.get_tip_block() == node1.get_tip_block()),);
        ...
        assert!(
            wait_until(10, || node0.get_tip_block().header().number()
                == node1.get_tip_block().header().number()),
            "Node1 should relay the new block to node0, and node0 should accept it"
        );
    }
}
