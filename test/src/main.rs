...
fn main() {
    ...
    for (index, (spec_name, spec)) in &mut specs_iter {
        ...
        let (net, nodes) = spec.prepare(&binary, start_port);
        let node_dirs: Vec<_> = nodes
            .iter()
...
fn tail_node_logs(node_dirs: Vec<String>) {
    const TAIL_LAST_LINES: usize = 2000;
    ...
    for (i, node_dir) in node_dirs.into_iter().enumerate() {
        ...
        let skip = content.lines().count().saturating_sub(TAIL_LAST_LINES);
