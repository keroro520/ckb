...
impl<CS: ChainStore> Synchronizer<CS> {
    ...
    pub fn eviction(&self, nc: &CKBProtocolContext) {
        let mut peer_states = self.peers_manager.state.write();
...
mod tests {
    ...
    fn test_sync_process() {
        ...
        assert_eq!(
            synchronizer1.peers_manager.get_best_known_header(peer1),
            synchronizer1.peers_manager.get_best_known_header(peer2)
        );

        let best_known_header = synchronizer1.peers_manager.get_best_known_header(peer1);
