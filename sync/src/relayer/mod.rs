...
use crate::types::{Filter, PeersManager, SyncSharedState};
...
impl<CS: ChainStore + 'static> Relayer<CS> {
    ...
    pub fn accept_block(&self, nc: &CKBProtocolContext, peer: PeerIndex, block: &Arc<Block>) {
        ...
        if ret.is_ok() {
            ...
            let mut known_blocks = self.peers_manager.known_blocks.lock();
    ...
    pub fn ask_for_txs(&self, nc: &CKBProtocolContext) {
        for (peer, peer_state) in self.peers_manager.state.write().iter_mut() {
